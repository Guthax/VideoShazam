...
def sliding_window(x, w, compare_func):
    ...
    wl = len(w)
    frames = [0, 0, 0]
    scores = [sys.maxint, sys.maxint, sys.maxint]
    minimum = sys.maxint
    frame = 0
    frame2 = 0
    frame3 = 0
    ...
    for i in range(frame, len(x) - wl):
        diff = compare_func(w, x[i:(i+wl)])
        if (diff < minimum) & (i > frame+q_duration*frame_rate):
            minimum = diff
            frame2 = i
    ...
    for i in range(frame2, len(x) - wl):
        diff = compare_func(w, x[i:(i+wl)])
        if (diff < minimum) & (i > frame2+q_duration*frame_rate):
            minimum = diff
            frame3 = i
...
for video in video_list:
    ...
    print 'Best match at:', frame[0]/frame_rate, 'seconds, with score of:', score[0]
    print 'Second best match at:', frame[1]/frame_rate, 'seconds, with score of:', score[1]
    print 'Third best match at:', frame[2]/frame_rate, 'seconds, with score of:', score[2]
